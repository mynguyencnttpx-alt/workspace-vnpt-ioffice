...
>
> **Cập nhật 24/09/2026 (v1.1 — mô hình hỗ trợ & định tuyến theo địa bàn; khách hàng xác nhận):** bỏ "Team", hiện **Tầng/Địa bàn + Phạm vi phụ trách**; 7 vai trò nội bộ (thêm Agent helpdesk, Hỗ trợ trung tâm, Hỗ trợ dịch vụ); thẻ ticket ở Tổng quan tính theo **phạm vi xem mặc định**; Hỗ trợ dịch vụ chỉ có nút hành động + Hoạt động gần đây. Nguồn: `SRS/trang-dau-noibo/SRS.md` v1.1.
...
| 4 | Vai trò | Label | ReadOnly | • Hiển thị vai trò hiện có (Agent tỉnh / Agent helpdesk / Hỗ trợ trung tâm / Hỗ trợ dịch vụ / Biên tập nội dung / Quản trị viên / Chủ quản dịch vụ). Chỉ Quản trị viên đổi được (`qt-phan-quyen`, UC56). |
| 5 | Tầng / Địa bàn | Label | ReadOnly | • Nhân viên hỗ trợ: Agent tỉnh → "Tỉnh X" (Địa bàn được gán), Agent helpdesk → "Helpdesk công ty", Hỗ trợ trung tâm → "Hỗ trợ trung tâm", Hỗ trợ dịch vụ → "Không thuộc tầng tiếp nhận"; Quản trị viên, Chủ quản dịch vụ, Biên tập nội dung: ẩn. Chỉ Quản trị viên đổi (`qt-tao-tai-khoan-noibo` / `qt-phan-quyen`). Thay ô "Team" của v1.0. |
| 6 | Phạm vi phụ trách | Label (danh sách) | ReadOnly | • Chỉ hiện với Agent tỉnh, Agent helpdesk, Hỗ trợ trung tâm, Hỗ trợ dịch vụ: liệt kê các dòng Dịch vụ × Đối tượng. Agent tỉnh: giới hạn quyền xem; vai trò khác: bộ lọc mặc định. Chỉ Quản trị viên đổi. |
...
| # | Items | Control type | Data type | Description |
|---|-------|--------------|-----------|-------------|
| 1 | Nút hành động chính | Button | Click | • Theo vai trò (đã chốt 23/09/2026): Agent tỉnh/Agent helpdesk/Hỗ trợ trung tâm/Hỗ trợ dịch vụ/Quản trị viên → "Xem hàng đợi ticket" (`agent-hang-doi`); Chủ quản dịch vụ → "Xem báo cáo" (`baocao-tong-quan`); Biên tập nội dung → "Xem chờ duyệt" (`kb-cho-duyet`). |
| 2 | Thẻ Ticket mới trong phạm vi của tôi | Stat card | Click | • Số ticket mới trong **phạm vi xem mặc định** của vai trò — gồm ticket giao cho tài khoản và ticket chưa phân công nằm trong phạm vi (kèm số khẩn cấp); bấm → `agent-hang-doi` đã lọc tương ứng. Chỉ Agent tỉnh, Agent helpdesk, Hỗ trợ trung tâm, Quản trị viên (đã chốt 24/09/2026). |
| 3 | Thẻ Sắp quá hạn SLA | Stat card | Click | • Số ticket còn dưới ngưỡng cảnh báo SLA (cấu hình ở `cauhinh-sla`); bấm → `agent-canh-bao-sla`. Chỉ Agent tỉnh, Agent helpdesk, Hỗ trợ trung tâm, Quản trị viên, trong phạm vi xem mặc định; **Hỗ trợ dịch vụ ẩn** (không nhận cảnh báo SLA). |
| 4 | Thẻ Khách vừa phản hồi | Stat card | Click | • Số ticket khách hàng vừa phản hồi cần trả lời; bấm → `agent-hang-doi` lọc trạng thái tương ứng. Hỗ trợ dịch vụ ẩn (vai trò chỉ xem + ghi chú nội bộ). |
...
| 6 | Việc cần làm ngay | List | Click | • Tối đa 3 mục ưu tiên (khẩn cấp, sắp quá hạn, khách phản hồi); bấm 1 dòng → `agent-chi-tiet-ticket`. Rỗng → "Chưa có việc cần làm". Ẩn với Hỗ trợ dịch vụ. |
...
- **Đề xuất bổ sung, đã chốt (OQ-25).** Màn này KHÔNG thay landing mặc định theo vai trò (OQ-18, cập nhật 24/09/2026: Agent tỉnh/helpdesk, Hỗ trợ trung tâm/dịch vụ, Quản trị viên → hàng đợi, Chủ quản → báo cáo tổng quan, Biên tập → chờ duyệt); mở từ menu "Tổng quan". Mọi thẻ hiển thị theo vai trò; lối sang `noibo-tai-khoan-ca-nhan` giữ nguyên.

#### Trạng thái phụ — Hỗ trợ dịch vụ

```text
┌──────────────────────────────────────────────────────────────────────┐
│ CSKH-NB  Tầng: (không có) | Vai trò: Hỗ trợ dịch vụ    (o) H v       │
├──────────────────────────────────────────────────────────────────────┤
│ Xin chào, Đỗ H                           [1] [ Xem hàng đợi ticket ] │
├──────────────────────────────────────────────────────────────────────┤
│ Hoạt động gần đây [7]                                                │
│ Bạn thêm ghi chú nội bộ vào #T-0123 (17/09 09:20)                    │
│ Phiếu OneBSS OB-2026-0456 của #T-0123 đã được tạo                    │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
```

- Khác màn gốc: chỉ còn nút hành động [1] và [7] Hoạt động gần đây (mục thuộc quyền của vai trò); ẩn cả 4 thẻ số liệu và "Việc cần làm ngay" (đã chốt 24/09/2026).
...
|----|-------------------|---------|------------|
| OQ-18 | Tài khoản nội bộ | Quản trị viên tạo → hệ thống gửi email mời đặt mật khẩu (không cấp mật khẩu qua kênh khác); có "quên mật khẩu" qua email; đăng nhập dùng chung một màn với khách hàng (OQ-32); trang đầu: Agent tỉnh/helpdesk, Hỗ trợ trung tâm/dịch vụ và Quản trị viên → hàng đợi ticket (cập nhật 24/09/2026), Chủ quản dịch vụ → báo cáo tổng quan, Biên tập → chờ duyệt. | Đã chốt (khách hàng xác nhận, 21/09/2026) |
