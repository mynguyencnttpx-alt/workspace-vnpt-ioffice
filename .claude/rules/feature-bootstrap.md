...
|---|---|---|
| **A — Điểm vào** (có thể khởi tạo feature) | `brainstorm`, `urd`, `brd`, `prd-epic`, `usecase` (discovery mode), `user-flow`, `bpmn`, `sequence`, `activity`, `activity-swimlane`, `erd`, `state`, `d2-activity`, `d2-erd`, `d2-architect`, `reverse-doc`, `code-to-srs` | **Derive slug + phỏng vấn đúng phạm vi + tạo feature.** Xem Mục "Nhóm A". (`reverse-doc` + `code-to-srs` là biến thể: derive slug từ NGUỒN/CODE thay vì phỏng vấn, xem ghi chú dưới.) |
| **B — Giữa/cuối chain** (cần artifact upstream làm nguồn) | `usecase-diagram`, `userstory`, `ac`, `figma`, `prototype`, `jira`, `confluence`, `export`, `preview`, `reverse-preview`, **họ API** (`api-assess`, `api-doc`, `api-design`, `api-map`, `api-checklist`, `api-test`, `api-readiness`) | **Refuse tường minh + route cụ thể về skill upstream.** KHÔNG tự tạo feature. Xem Mục "Nhóm B". |
| **C — Read-only / project-level** (không cần feature) | `dashboard`, `gap`, `review`, `prd`, `roadmap`, `meet`, `discover`, `update-overview`, `delegate`, `userguide`, `doc-drift` | **Friendly empty-message.** Xem Mục "Nhóm C". |
...
> **Họ API (7 skill) — nhóm B, luôn trong 1 feature đã có.** Không skill API nào tự khởi tạo feature: chúng gắn vào feature đang tồn tại (`docs/{feature}/`) và cần upstream theo pipeline `api-integration.md` (`api-assess → api-doc → api-design → api-map → api-checklist → api-test → api-readiness`). Feature chưa có → **refuse + route** `/brainstorm`/`/srs` để tạo feature trước. Trong feature đã có nhưng thiếu upstream trực tiếp → refuse + route đúng bước trước (vd `/api-test` thiếu checklist → `/api-checklist`; `/api-design` thiếu hiểu contract → `/api-doc`). `/api-doc` cần thêm **nguồn** (OpenAPI/PDF/URL) — thiếu nguồn thì hỏi nguồn, KHÔNG phỏng vấn derive-slug kiểu nhóm A.
...
5. `doc-drift`: read-only ra report so code↔docs — nhưng CẦN **docs baseline** (`docs/{feature}/`) làm nguồn
