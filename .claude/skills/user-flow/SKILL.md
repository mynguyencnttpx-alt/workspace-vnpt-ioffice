...
* __HARD STOP sau khi flow được duyệt.__ Skill này CHỈ dừng ở user flow — KHÔNG vẽ wireframe (đó là việc của `/wireframe-ascii` hoặc `/wireframe-html`, gọi riêng hoặc qua `/srs-baket`).
...
* **`flow_hash` là cơ chế chống hỏi lại** — Phase A.5 dùng nó để biết flow đã duyệt chưa, tránh chạy lại toàn bộ phân tích mỗi lần `/wireframe-ascii` hay `/srs-baket` cần đọc userflow.
...
[Phase F.5] Render-verify mermaid (mermaid-verify.mjs) → fail thì tự sửa ≤2 lần.
        │
        ▼
[Phase G] Final report + recommend next (/wireframe-ascii, /wireframe-html, /srs-baket).
```
...
Recommended next:
  ...
  - /srs-baket {feature}                — kỹ thuật hoá thành SRS đầy đủ
```
...
* @../../rules/resolve-oqs.md (chỉ áp dụng TINH THẦN "thà ghi Open Question còn hơn bịa" — user-flow là điểm-vào sớm, KHÔNG chạy Phase resolve-OQ đầy đủ; resolve OQ là việc của `/urd` `/srs-baket` sau)
