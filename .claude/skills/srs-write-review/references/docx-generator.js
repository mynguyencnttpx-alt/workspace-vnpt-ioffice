...
function workflowImage(pngPath = 'workflow.png', dispW = DISP_W, dispH = DISP_H) {
  if (!fs.existsSync(pngPath)) {
    console.warn(`⚠️  ${pngPath} không tìm thấy — bỏ qua hình minh họa quy trình`);
    return para('[Không có hình minh họa quy trình — xem Luồng xử lý thành công bên dưới]', { italic: true });
  }
...
// ─── WORKFLOW STEPS (đoạn văn + gạch đầu dòng) — ① Luồng xử lý thành công ────
// MODEL: thay workflowSteps bằng các bước thực tế.
// Mỗi bước = 1 khối: "[Bước N:] <Actor> <hành động>, hệ thống thực hiện:" + bullet các xử lý.
// Chỉ 1 bước/trigger duy nhất → không cần đánh số. Nhiều bước nối tiếp → đánh số Bước 1, Bước 2...
// Ghi ngắn "Gửi thông báo" nếu có — chi tiết để ở notificationTable().

const workflowSteps = [
  {
    actor: 'Người dùng',
    action: '<mô tả hành động — ngắn gọn, Động từ + đối tượng>',
    processing: ['<Hệ thống xử lý 1 — validate, lưu DB, v.v.>', '<Hệ thống xử lý 2>'],
  },
];

function workflowStepBlocks() {
  const multiStep = workflowSteps.length > 1;
  const blocks = [];
  workflowSteps.forEach((s, i) => {
    const prefix = multiStep ? `Bước ${i + 1}: ` : '';
    const actorLabel = s.actor || 'Người dùng';
    blocks.push(para(`${prefix}${actorLabel} ${s.action}, hệ thống thực hiện:`, { before: i === 0 ? 60 : 140 }));
    s.processing.forEach(p => blocks.push(bullet(p)));
  });
  return blocks;
}
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      // B.1 Quy trình
      h4('Quy trình'),
      para('[Vẽ Sequence Diagram nếu chức năng có quy trình nhiều bước / nhiều actor tương tác qua lại. Bỏ qua nếu chỉ là chỉnh sửa nhỏ.]', { italic: true }),
